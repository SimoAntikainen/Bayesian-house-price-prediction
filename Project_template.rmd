...
#plot(density(replicated_data_lin))
#plot(density(replicated_data_pol))
#plot(density(original_target))
ggplot(ppc_data) +
  geom_density()
...
ppc_loo_pit_overlay(c(original_target[training_indices]), yrep = replicated_data_lin,
                    lw = lw_lin)
ppc_loo_ribbon(c(original_target[training_indices]), yrep = replicated_data_lin,
...
ppc_loo_pit_overlay(c(original_target[training_indices]), yrep = replicated_data_pol,
                    lw = lw_lin)
ppc_loo_ribbon(c(original_target[training_indices]), yrep = replicated_data_pol,
               lw = lw_lin, psis_object = psis_lin)
ppc_loo_intervals(c(original_target[training_indices]),
                  yrep = replicated_data_pol, psis_object = psis_lin)
```


# 6. Model comparison

Using leave one out (LOO), the models have no pareto-k values over 0.7, showing that the estimates are reliable.
