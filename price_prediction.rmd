...
[6] https://mc-stan.org/users/documentation/case-studies/radon.html
...
We train the models on 80%/20%-test split using 10000 first datapoints. Separate models are fitted for all the 71 zipcodes.
...
```{r}
result_testing = data.frame(price = exp(testing_target), predicted = exp(predicted_prices))
...
```{r}
p <- plot(multiple_linear_fit, pars = "beta", regex_pars = "period",  
...
```{r}
p <- plot(multiple_polynomial_fit, pars = "beta", regex_pars = "period",  
          prob = 0.5, prob_outer = 0.9)
p + ggplot2::ggtitle("Posterior medians \n with 50% and 90% intervals")
```
